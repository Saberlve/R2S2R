...
from real2sim.traj.replay_metrics import bar_lift_report
...
tactile_provenance = None
if tactile:
    from real2sim.tactile.run import TactileRunWriter

    # The writer is opened before the run starts, so a failure anywhere below still leaves a
    # manifest beside whatever chunks were written.
...
try:
    if tactile:
        ...
        mechanics_backend = build_hydroshear_backend(
            facade, indenter_object_ids=["bar"]
        )
    rows = []
    actual = []
    targets = []
    ...
    if a.bar:
        # A frame count is not a duration: the tactile configuration replays the same episode at
        # a different control rate, so the hold is measured against the recorded timestamps.
        report.update(bar_lift_report(obj, settings["table_matrix"], tlist))
    if contact_log:
    ...
    if tactile_writer is not None:
        tactile_writer.finalize(tactile_provenance)
    ...
    if tactile_writer is not None:
        try:
            # The frames that did complete keep their indices, times and contact rows, so a
            # failed run stays analysable instead of being merely absent.
            tactile_writer.finalize(tactile_provenance, status="failed", error=repr(exc))
        except Exception as manifest_error:
            print(
                "Tactile failure manifest could not be written:",
                manifest_error,
                flush=True,
            )
    raise
