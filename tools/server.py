"""Headless research entry points. Uses configured runtimes; never contacts hardware."""
...
def main():
 ap=argparse.ArgumentParser(description=__doc__);ap.add_argument('--site',default=os.environ.get('R2S_SITE_CONFIG',str(ROOT/'site.local.json')))
 ...
 for name in ['doctor','smoke','inspect','edit','render','simulate']:
  ...
  if name in ['inspect','edit','render','simulate']:p.add_argument('--template',help='Runtime directory; defaults to site reference_template')
 ...
 a=ap.parse_args();site=load(a.site);env=os.environ.copy();env.update(PYTHONPATH=str(ROOT/'src')+os.pathsep+env.get('PYTHONPATH',''),OPENBLAS_NUM_THREADS='1',OMP_NUM_THREADS='8')
